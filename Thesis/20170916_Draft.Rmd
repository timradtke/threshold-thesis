...
Consider the problem the latter companies face. Whenever a user reports that she visited a restaurant, the app would like to ask questions about her experience. To give a simple example, assume users are not willing to answer more than one question per restaurant visit, and the app would like to find out two things: Does the restaurant have good service? Is the restaurant trendy? See \autoref{sec:Foursquare} for what this information looks like in the Foursquare app.
...
The previous chapter introduces the thresholding bandit problem as described in Locatelli et al. (2016), and the APT strategy as a first algorithm that is simple, yet order-optimal for large budgets. However, we end with the conclusion that a KL-divergence based measure of the distance between the distributions and the threshold might be more useful than the simple gap $\Delta^2_i = (\mu_i - \tau_i)^2$ employed so far. Indeed, citing Kaufmann et al. (2016), we note that the complexity $H$ -- and thus the gaps -- used so far can be considered a sub-Gaussian approximation of a better complexity measure based on information theoretic measures like the Kullback-Leibler divergence.
...
Before we discuss algorithms given in Kaufmann et al. (2016) that match the lower bounds, we show how based on the proof of Theorem 12 in Kaufmann et al. (2016) very similar lower bounds can be derived for the fixed budget thresholding problem.
...
We present the proofs of \autoref{sec:LBTBP} in the appendix in \autoref{sec:corollaryLBTBPproof}. While we were hoping to also prove a lower bound for the $K$-armed bandit problem, we were not able to do so as this entails the expectation under the alternative problem of the number of times the changed arm $a$ is pulled. Compare to the proof for the two-armed pure exploration bandit in Kaufmann et al. (2016).
...
$$
\end{corollary} Corresponding to the complexity measures defined in equation $(1)$ of Kaufmann et al. (2016), we can define the complexity $\kappa_{TBP}(\nu)$ in the thresholding bandit problem and lower bound it:
...
This lower bound can be interpreted as a minimax game^[See the presentation by Aurélien Garivier at COLT 2016: <https://youtu.be/eLyrrYqJlkE>], where the experimenter has to choose the share of samples she attributes to each arm in a way that is optimal given that nature will respond and choose the most difficult alternative model given her weights. Thus, to make the lower bound practical, it is a matter of finding the optimal weights $w^*(\bm{\mu})$. This step did not occur in the lower bound for the top-$m$ problem since the alternative models were specified by changing the distribution of one arm with respect to the "border" between the two groups of arms.
...
We have $B_i^{LR}(t) \leq 1$, $t \uparrow \rightarrow B_i^{LR}(t) \downarrow$, and $|\hat{\mu}_i(t) - \tau| \uparrow \rightarrow B_i^{LR}(t) \downarrow$. Consequently, by choosing the arm that maximizes $B_i^{LR}(t)$, we choose an arm for which we either have not collected many samples, or for which the sample mean is close to the threshold $\tau$. In this sense, a strategy based on the likelihood ratio is reminiscient of the index employed by the APT algorithm. Indeed, assuming the samples are normally distributed with equal variance for all arms under both $H_0$ and $H_1$, $-\log(B_i^{LR}(t))$ equals the index in APT. Why then would we prefer the likelihood based strategy?
...
As soon as we move away from one-parameter distributions parameterized by their mean, it becomes less clear how the likelihood ratio for $H_0: \mu = \tau$ and $H_1: \mu = \hat{\mu}$ should be calculated. In the case of Normal distributions with unknown mean and unknown variance, given that the variance and mean estimate are linked, we can use 
$$
...
In the thesis at hand, we have introduced the general thresholding bandit problem introduced by Locatelli et al. (2016), as well as their Anytime Parameter-Free Thresholding algorithm that tries to solve the problem optimally. Given a few limitations of the APT algorithm, we explored the literature on top-$m$ multi-armed bandit problems to see how the Kullback-Leibler divergence can be used to improve algorithms and performance bounds. Based on this we exhibit the similarity between the thresholding bandit problem and simple hypothesis testing. We derive a lower bound on the complexity of a one-armed thresholding bandit that corresponds to the Chernoff-Stein Lemma. Continuing with the intuition from hypothesis testing, we propose the new Simple Likelihood Ratio algorithm for the multi-armed thresholding bandit for one-parameter exponential family of distributions. We show in extensive simulations that the the empirical performance of the SLR algorithm improves significantly upon the APT algorithm. We show that adding the D-Tracking Rule originally proposed by Garivier and Kaufmann (2016) can eradicate some of the remaining weaknesses. We propose a new method of testing thresholding bandit algorithms on real data by treating it as in time series cross validation. We show that the adaptive sampling strategies are not robust to the time dependency in the data set.

Problems for future work are the derivation of lower and upper bounds on the error probability of the SLR algorithm. Furthermore, one might be able to adapt the lower bound on the complexity of the one-armed thresholding bandit to the multi-armed version. Lastly, the surprisingly very good performance of the Bayes-UCB algorithm in our simulations, as well as its better sample allocation to individual arms in terms of concentration compared to APT and SLR, justify more research effort, and a theoretical analysis of its performance.
...
O. Capp´e, A. Garivier, O-A. Maillard, R. Munos, and G. Stoltz. Kullback-Leibler upper confidence bounds for optimal sequential allocation. *Annals of Statistics*, 41(3):1516–1541, 2013a.\par

O. Capp´e, A. Garivier, O.-A. Maillard, R. Munos, G. and Stoltz. Supple-
ment to “Kullback–Leibler upper confidence bounds for optimal sequential allocation.”, *Annals of Statistics*, <DOI:10.1214/13-AOS1119SUPP>, 2013b. \par
...
E. Kaufmann, O. Capp´e, and A. Garivier. On Bayesian Upper Confidence Bounds for Bandit Problems. In *Proceedings of the 15th International Conference on Artificial Intelligence and Statistics (AISTATS)*, 2012.\par

E. Kaufmann, O. Capp´e, A. Garivier. On the Complexity of Best Arm Identification in Multi-Armed Bandit Models. Journal of Machine Learning Research, 17:1-42, 2016.\par
...
## Example of User Feedback in Foursquare App \label{sec:Foursquare}

![The right screenshot shows aggregated user feedback in the Foursquare app. One could use thresholding bandit algorithms as presented in this thesis to ask different questions so as to optimize the confidence with which the results are displayed for a restaurant in the app.](figures/Foursquare_Screenshot.png)
