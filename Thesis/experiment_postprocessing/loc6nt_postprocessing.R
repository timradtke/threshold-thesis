...
load(paste0(current_path, "loc6nt_BUCB_horizon_c5_7000.Rda"))
loc6nt_BUCB_c5_arm_seq <- data.frame(t(laply(loc6nt_BUCB_horizon_c5_7000, 
                                          function(x) x$arm_sequence)))
save(loc6nt_BUCB_c5_arm_seq, file = "/Users/timradtke/Dropbox/1Master/Master Thesis/threshold-thesis/Thesis/experiment_postprocessing/loc6nt_BUCB_c5_arm_seq.Rda")
rm(loc6nt_BUCB_horizon_c5_7000, loc6nt_BUCB_c5_arm_seq)
gc()

load(paste0(current_path, "loc6nt_BUCB_horizon_c15_7000.Rda"))
loc6nt_BUCB_c15_arm_seq <- data.frame(t(laply(loc6nt_BUCB_horizon_c15_7000, 
                                          function(x) x$arm_sequence)))
save(loc6nt_BUCB_c15_arm_seq, file = "/Users/timradtke/Dropbox/1Master/Master Thesis/threshold-thesis/Thesis/experiment_postprocessing/loc6nt_BUCB_c15_arm_seq.Rda")
rm(loc6nt_BUCB_horizon_c15_7000, loc6nt_BUCB_c15_arm_seq)
gc()
...
load("/Users/timradtke/Dropbox/1Master/Master Thesis/threshold-thesis/Thesis/experiment_postprocessing/loc6nt_BUCB_arm_seq.Rda")

loc6nt_BUCB_arm_seq_summary <- loc6nt_BUCB_arm_seq %>% 
  tbl_df() %>% 
  mutate(index = 1:7000) %>%
  gather(key = iter, value = arm, -index) %>%
  dplyr::filter(index > 10) %>%
...
names(loc6nt_BUCB_arm_seq_summary)[-1] <- c(paste0("V0", 1:9), "V10")

loc6nt_BUCB_ma <- data.frame(algorithm = rep("BUCB_C1",6981),
                           index = 1:6981,
                           lapply(loc6nt_BUCB_arm_seq_summary[,-1], FUN = zoo::rollapply, width = 10, mean)) %>%
  gather(arm, mean, -index, -algorithm)

load("/Users/timradtke/Dropbox/1Master/Master Thesis/threshold-thesis/Thesis/experiment_postprocessing/loc6nt_BUCB_c5_arm_seq.Rda")

loc6nt_BUCB_c5_arm_seq_summary <- loc6nt_BUCB_c5_arm_seq %>% 
  tbl_df() %>% 
  mutate(index = 1:7000) %>%
  gather(key = iter, value = arm, -index) %>%
  dplyr::filter(index > 10) %>%
...
names(loc6nt_BUCB_c5_arm_seq_summary)[-1] <- c(paste0("V0", 1:9), "V10")

loc6nt_BUCB_c5_ma <- data.frame(algorithm = rep("BUCB_C5",6981),
                             index = 1:6981,
                             lapply(loc6nt_BUCB_c5_arm_seq_summary[,-1], FUN = zoo::rollapply, width = 10, mean)) %>%
  gather(arm, mean, -index, -algorithm)

load("/Users/timradtke/Dropbox/1Master/Master Thesis/threshold-thesis/Thesis/experiment_postprocessing/loc6nt_BUCB_c15_arm_seq.Rda")

loc6nt_BUCB_c15_arm_seq_summary <- loc6nt_BUCB_c15_arm_seq %>% 
  tbl_df() %>% 
  mutate(index = 1:7000) %>%
  gather(key = iter, value = arm, -index) %>%
  dplyr::filter(index > 10) %>%
...
names(loc6nt_BUCB_c15_arm_seq_summary)[-1] <- c(paste0("V0", 1:9), "V10")

loc6nt_BUCB_c15_ma <- data.frame(algorithm = rep("BUCB_C0.2",6981),
                                index = 1:6981,
                                lapply(loc6nt_BUCB_c15_arm_seq_summary[,-1], FUN = zoo::rollapply, width = 10, mean)) %>%
  gather(arm, mean, -index, -algorithm)


rbind(loc6nt_BUCB_ma, loc6nt_BUCB_c5_ma, loc6nt_BUCB_c15_ma) %>%
  ggplot(aes(x = index, y = mean, group = arm, color = arm)) +
  geom_line(size = 0.3) +
  facet_grid(.~algorithm) +
  labs(x = "Index", y = "10 Rounds Moving Average",
