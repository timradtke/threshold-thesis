...
random variable \(X_i\). We assume that \(X_i\) follows some
distribution \(Q(\theta)\) paramaterized by a vector \(\theta\). In a
standard hypothesis test, we would then want to test whether an
...
both parameterizations: \(L_{H_0} = L(x|\theta = \theta_{H_0})\) and
\(L_{H_1} = L(x|\theta = \theta_{H_1})\). The likelihood ratio test
statistic is then given as

\[
LR(\theta_{H_0}|| \theta_{H_1}) = \frac{L_{H_0}}{L_{H_1}}.
