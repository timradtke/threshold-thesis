...
tau_gex <- 0
epsilon_gex <- 0
(mu_gex <- rnorm(10, 0, 0.4))
(sd_gex <- sqrt(1/rgamma(10, 2, 0.1)))
...
system.time(gex_LR <- para_bandit_sim_LR_gaussian(data = data_gex, 
                                                   rounds = 10000, 
                                                   tau = tau_gex, 
                                                   epsilon = epsilon_gex))
...
system.time(gex_APT <- para_bandit_sim_APT(data = data_gex, 
                                            rounds = 10000, 
                                            tau = tau_gex, 
                                            epsilon = epsilon_gex,
                                            do_verbose = TRUE))
