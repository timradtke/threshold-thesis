...
for(j in 1:5000) {
  curr_data <- data.frame(rep(NA, times = 5000))
  for(i in 1:length(mu_pois1)) {
    curr_data[[i]] <- as.numeric(rpois(5000, mu_pois1[i]))
  }
...
pois1_comp_UNIFORM <- compare_to_ground_truth(mu_pois1, 
                                             pois1_UNIFORM, 
                                             tau_pois1, 
                                             epsilon_pois1)$mean
...
pois1_comp_AugUCB <- compare_to_ground_truth(mu_pois1, pois1_AugUCB, 
                                            tau_pois1, epsilon_pois1)$mean
